...
export default function Frame() {
  ...
  async function safeGetDates(user: `0x${string}`) {
    let count = 20n
    while (count > 0n) {
      try {
    ...
    <>
      ...
      <main className="p-6 space-y-6">
        ...
        <nav className="flex gap-4 text-emerald-700 font-medium">
          <button onClick={() => setView("entries")}>📖 Записи</button>
          <button onClick={() => setView("log")}>➕ Добавить</button>
          <button onClick={() => setView("chart")}>📊 График</button>
        </nav>
        ...
        {/* График (позже можно вставить Recharts) */}
        {view === "chart" && (
          <p className="text-gray-500">📊 График скоро будет</p>
        )}
      </main>
    </>
  )
